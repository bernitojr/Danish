...
export function useGameLog(gameState: GameState | null, isPlayerTurn: boolean) {
  ...
  useEffect(() => {
    ...
    if (gameState.pile.length > prev.pile.length) {
      ...
      if (top)
        push(
          `${who.name} joue ${top.rank}${SUIT[top.suit as keyof typeof SUIT]}`
        )
  ...
  useEffect(() => {
    if (isPlayerTurn) push('→ À toi de jouer !')
  }, [isPlayerTurn])
